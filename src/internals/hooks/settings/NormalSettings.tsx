...
export default ({ setPage }) => {
    ...
    return (
            ...
            <InfoContainer icon="fa-pencil-alt" title="Edit Info"> 
                    <div>
                        <a onClick={() => {
                            BPP.Plugins["Internals"].settings.hooks.changeUsername();
                        }} className="styles__link___5UR6_-camelCase">Change Username</a>
                    </div>
                    <div>
                        <a onClick={() => {
                            BPP.Plugins["Internals"].settings.hooks.changePass();
                        }} className="styles__link___5UR6_-camelCase">Change Password</a>
                    </div>
                    <div>
                        <a onClick={() => {
                            BPP.Plugins["Internals"].settings.hooks.otp();
            ...
            <InfoContainer icon="fa-cog" title="General">
                <div>
                    <a onClick={() => {
                        BPP.Plugins["Internals"].settings.hooks.tradeRequests();
                ...
                <div>
                    <a onClick={() => {
                        BPP.Plugins["Internals"].settings.hooks.friendRequests();
            ...
            <div>
                <a onClick={() => {
                    BPP.Plugins["Internals"].settings.hooks.changeUsernameColor()
                }} id="changeUsernameColorButton" className="styles__link___5UR6_-camelCase">Change Username Color</a><br/>
                <a onClick={() => {
                    BPP.Plugins["Internals"].settings.hooks.changeDefaultChatColor()
